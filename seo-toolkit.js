...
(function(){
  ...
  window.addEventListener('mousemove', e => {
    ...
    // Spawn a soft glow trail dot every ~40ms, not on every single move event
    const now = Date.now();
    if(now - lastTrailTime > 40){
      lastTrailTime = now;
      const t = document.createElement('div');
      t.className = 'trail-glow';
  ...
  // Highlight over interactive elements
  ...
  // Subtle pulse + expanding ripple on click
  ...
  document.addEventListener('mouseup', () => arrow.classList.remove('clicking'));
})();
...
// ---------- FETCH & ANALYZE (calls the Vercel serverless function) ----------
async function fetchAndAnalyze(){
  const url = document.getElementById('check-url').value.trim();
  const statusEl = document.getElementById('fetch-status');

  if(!url){
    statusEl.innerText = 'Enter a page URL first.';
    return;
  }

  statusEl.innerText = 'Fetching page…';

  try{
    const res = await fetch('/api/fetch?url=' + encodeURIComponent(url));
    if(!res.ok){
      const err = await res.json().catch(()=>({error:'Unknown error'}));
      statusEl.innerText = `Couldn't fetch that page (${err.error || res.status}). Try pasting the source manually below instead.`;
      return;
    }
    const html = await res.text();
    document.getElementById('check-html').value = html;
    statusEl.innerText = 'Fetched successfully — analyzing…';
    runChecklist();
    statusEl.innerText = 'Done.';
  }catch(e){
    statusEl.innerText = 'Network error — try pasting the source manually below instead.';
  }
}
